  ...
  return user ? (
    <div className="flex flex-wrap flex-col py-10 w-10/12 self-center">
      <div className="self-center">
        <button
          className="rounded-lg register-bg h-12 w-32 text-2xl font-bold mb-8 shadow-md shadow-slate-900"
          onClick={() => {
            navigate("/newpost");
          }}
        >
          ...
          return (
            <div className="dark:darkpost-bg post-bg border-2 rounded-lg">
              <img
                ...
                src={post.image}
              />
              <p className="text-2xl darkpost-bg dark:register-bg scroll-my-1 border-1 border-black rounded-lg p-4 font-semibold">
                {post.content}
              </p>
              ...
              </form>
              {
                <div className="self-center border-1 border-black rounded-lg">
                  {post.comments.map((comment) => {
                    return (
                      <div key={comment.id} className="border-2 rounded-lg">
                        <p className="flex flex-row text-xl dark:register-bg darkpost-bg max-h-fit p-4 font-semibold">
                        <img onClick={()=>{viewProfile(comment.user)}} className="border-green-500 border-4 hover:cursor-pointer dark:border-primary rounded-lg h-20 w-20" src={image(comment.user)}/>
                          <span className="mx-5 self-center">{comment.content}</span>
                        </p>
                      </div>
                    );
                  })}
